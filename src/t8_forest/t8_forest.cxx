  ...
  for (eclass_it = T8_ECLASS_VERTEX; eclass_it < T8_ECLASS_COUNT; eclass_it++) {
    if (forest->cmesh->num_trees_per_eclass[eclass_it] > 0) {
      ...
      if (forest->set_type == 1) {
        ...
        const t8_mixed_scheme *scheme = (const t8_mixed_scheme *) t8_forest_get_scheme_before_commit (forest);
        maxlevel = scheme->get_maxlevel ((t8_eclass_t) eclass_it);
      }
...
t8_forest_populate (t8_forest_t forest)
{
  ...
  if (forest->set_type == 1) {
    ...
    /* Note: until this point the horizontal maximum refinement level equals the  vertical maximum refinement level*/
    SC_CHECK_ABORT (forest->set_level1 <= forest->maxlevel, "Given refinement level exceeds the horizontal maximum.\n");
    SC_CHECK_ABORT (forest->set_level2 <= forest->maxlevel, "Given refinement level exceeds the vertical maximum.\n");
  }
...
/* This is the search for 2.5 dimensions.
 ...
 */
static t8_locidx_t
t8_forest_bin_search_lower_2_5D (const t8_element_array_t *elements, const t8_linearidx_t element_id,
                                 std::vector<int> maxlevels)
{
...
t8_forest_element_is_leaf (const t8_forest_t forest, const t8_element_t *element, const t8_locidx_t local_tree)
{
  ...
  if (forest->set_type == 1) {
    ...
    * such that the element at position i has a smaller id than the given one.
    * If no such i exists, it returns -1. */
    const t8_locidx_t search_result = t8_forest_bin_search_lower_2_5D (elements, element_id, levels);
    if (search_result < 0) {
      /* The element was not found. */
      return 0;
    }
    ...
    * To identify whether the element was found, we compare these two. */
    ...
    T8_ASSERT ("The type of the forest must be either 1 for the default scheme or 2 for the 2.5 dimensional scheme.\n");
    return 0;
  }
}
                            ...
                            const int do_face_ghost, sc_MPI_Comm comm)
{
  ...
  if (cmesh->set_partition) {
    T8_ASSERT ("t8_forest_new_uniform_2_5D isn't implemented for a partiotioned cmesh yet.\n");
  }
